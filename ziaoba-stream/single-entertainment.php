                    ...
                    <?php else :
                        ...
                        if ( $episodes ) :
                            ...
                            foreach ( $episodes as $episode ) {
                                $season_key = (int) get_post_meta( $episode->ID, '_ziaoba_season', true );
                                $season_key = $season_key > 0 ? $season_key : 1;
                                if ( ! isset( $episodes_by_season[ $season_key ] ) ) {
                                    $episodes_by_season[ $season_key ] = array();
                                }
                                $episodes_by_season[ $season_key ][] = $episode;
                            }
                            ksort( $episodes_by_season );
                            $season_keys     = array_keys( $episodes_by_season );
                            $default_season  = ! empty( $season_keys ) ? (string) end( $season_keys ) : 'all';
                            ?>
                            <section class="episodes-section series-browser">
                                <div class="episodes-header">
                                    <div>
                                        <h2 class="section-title"><?php esc_html_e( 'Browse Episodes', 'ziaoba-stream' ); ?></h2>
                                        <p class="section-subtitle"><?php esc_html_e( 'Choose a season, then jump directly into any episode.', 'ziaoba-stream' ); ?></p>
                                    </div>
                                    <select id="seasonSelector" class="season-select" aria-label="<?php esc_attr_e( 'Choose season', 'ziaoba-stream' ); ?>">
                                        <?php foreach ( $season_keys as $season_key ) : ?>
                                            <option value="<?php echo esc_attr( $season_key ); ?>" <?php selected( (string) $season_key, $default_season ); ?>><?php echo esc_html( sprintf( __( 'Season %s', 'ziaoba-stream' ), $season_key ) ); ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>

                                <div class="season-pill-row" role="tablist" aria-label="<?php esc_attr_e( 'Season selection', 'ziaoba-stream' ); ?>">
                                    <?php foreach ( $season_keys as $season_key ) : ?>
                                        <button type="button" class="season-pill<?php echo (string) $season_key === $default_season ? ' is-active' : ''; ?>" data-season-target="<?php echo esc_attr( $season_key ); ?>" role="tab" aria-selected="<?php echo (string) $season_key === $default_season ? 'true' : 'false'; ?>">
                                            <?php echo esc_html( sprintf( __( 'Season %s', 'ziaoba-stream' ), $season_key ) ); ?>
                                        </button>
                                    <?php endforeach; ?>
                                </div>

                                <div class="season-panels">
                                    <?php foreach ( $episodes_by_season as $season_key => $season_episodes ) : ?>
                                        <section class="season-panel<?php echo (string) $season_key === $default_season ? ' is-active' : ''; ?>" data-season-panel="<?php echo esc_attr( $season_key ); ?>">
                                            <div class="season-panel-header">
                                                <h3><?php echo esc_html( sprintf( __( 'Season %s', 'ziaoba-stream' ), $season_key ) ); ?></h3>
                                                <span><?php echo esc_html( sprintf( _n( '%s episode', '%s episodes', count( $season_episodes ), 'ziaoba-stream' ), number_format_i18n( count( $season_episodes ) ) ) ); ?></span>
                                            </div>
                                            <div class="episodes-grid" id="episodesGrid-<?php echo esc_attr( $season_key ); ?>">
                                                <?php foreach ( $season_episodes as $episode ) : ?>
                                                    ...
                                                    ?>
                                                    <article class="episode-card poster-slide" data-season="<?php echo esc_attr( $season_key ); ?>" onclick="location.href='<?php echo esc_url( get_permalink( $episode->ID ) ); ?>'">
                                                        <div class="card-img-wrapper poster-card-img-wrapper">
                                                            ...
                                                            <?php if ( $ep_num ) : ?><div class="ep-badge"><?php echo esc_html( sprintf( 'E%s', $ep_num ) ); ?></div><?php endif; ?>
                                                        </div>
                                                        <div class="card-info">
                                                            <h3 class="card-title"><?php echo esc_html( get_the_title( $episode->ID ) ); ?></h3>
                                                            <div class="card-meta wrap-meta">
                                                                <?php if ( $ep_duration ) : ?><span><?php echo esc_html( $ep_duration ); ?></span><?php endif; ?>
