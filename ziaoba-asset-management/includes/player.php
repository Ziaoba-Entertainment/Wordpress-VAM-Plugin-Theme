...
function ziaoba_validate_stream_token( $token, $post_id, $user_id, $target = '' ) {
    ...
    if ( ! array_key_exists( 'target', $decoded ) || (string) $decoded['target'] !== $target ) {
        return false;
    }
...
function ziaoba_get_stream_proxy_url( $post_id, $target ) {
    $target = (string) $target;
    $args   = array(
        'action'  => 'ziaoba_stream_video',
        'post_id' => (int) $post_id,
        'token'   => ziaoba_generate_stream_token( $post_id, get_current_user_id(), $target ),
    );
...
function ziaoba_rewrite_manifest_contents( $manifest, $post_id, $base_url ) {
    ...
    foreach ( $lines as $line ) {
        ...
        if ( 0 === strpos( $trimmed, '#EXT-X-KEY:' ) ) {
            $rewritten[] = preg_replace_callback(
                ...
                function( $matches ) use ( $post_id, $base_url ) {
                    ...
                    $proxy = ziaoba_get_stream_proxy_url( $post_id, $absolute );
        ...
        $rewritten[] = ziaoba_get_stream_proxy_url( $post_id, $absolute );
    }
