...
class TMDB {
     ...
     */
    private static function fetch_tmdb( $endpoint, $params = array() ) {
        $cache_key = 'ziaoba_tmdb_' . md5( $endpoint . serialize( $params ) );
        ...
        $url = add_query_arg( array_merge( array( 'api_key' => self::$api_key ), $params ), self::$api_url . $endpoint );
        
        error_log( "Ziaoba TMDB: Fetching URL: " . $url );

        $response = wp_remote_get( $url, array( 'timeout' => 15 ) );

        if ( is_wp_error( $response ) ) {
            error_log( "Ziaoba TMDB Error: " . $response->get_error_message() );
            return false;
        }

        $code = wp_remote_retrieve_response_code( $response );
        if ( $code !== 200 ) {
            error_log( "Ziaoba TMDB Error: HTTP Code " . $code );
            return false;
        }
        ...
        if ( ! $data ) {
            error_log( "Ziaoba TMDB Error: Invalid JSON response" );
            return false;
        }
    ...
    public function ajax_search() {
        ...
        $data = self::fetch_tmdb( "/search/{$type}", array( 'query' => $query ) );
    ...
    public function ajax_import() {
        ...
        $data = self::fetch_tmdb( "/{$type}/{$tmdb_id}", array( 'append_to_response' => 'release_dates,content_ratings,videos,credits' ) );
        ...
        // Update Meta
        update_post_meta( $post_id, '_ziaoba_tmdb_id', $tmdb_id );
        update_post_meta( $post_id, '_tmdb_id', $tmdb_id ); // Legacy support
        ...
        if ( isset( $data['videos']['results'] ) ) {
            foreach ( $data['videos']['results'] as $video ) {
                if ( $video['site'] === 'YouTube' && ( $video['type'] === 'Trailer' || $video['type'] === 'Teaser' ) ) {
                    ...
                    update_post_meta( $post_id, '_ziaoba_trailer_url', $trailer_url );
                    update_post_meta( $post_id, '_trailer', $video['key'] ); // Legacy support
                    break;
                }
            }
        }
        ...
        if ( $age_rating ) {
            update_post_meta( $post_id, '_ziaoba_age_rating', $age_rating );
            update_post_meta( $post_id, '_age_rating', $age_rating ); // Legacy support
        }
        ...
        if ( isset( $data['poster_path'] ) ) {
            ...
            update_post_meta( $post_id, '_ziaoba_poster_url', $poster_url );
    ...
    public function ajax_fetch_seasons() {
        ...
        $data = self::fetch_tmdb( "/tv/{$tmdb_id}" );
        ...
        foreach ( $data['seasons'] as $season_data ) {
            ...
            if ( $season_data['poster_path'] ) {
                ...
                update_post_meta( $season_post_id, '_ziaoba_poster_url', $poster_url );
    ...
    public function ajax_fetch_episodes() {
        ...
        $data = self::fetch_tmdb( "/tv/{$series_tmdb_id}/season/{$season_num}" );
        
        if ( ! $data || empty( $data['episodes'] ) ) {
    ...
    private function extract_certification( $data, $type ) {
        ...
        if ( $type === 'movie' && isset( $data['release_dates']['results'] ) ) {
            ...
            foreach ( $data['release_dates']['results'] as $res ) {
                if ( $res['iso_3166_1'] === 'US' && ! empty( $res['release_dates'][0]['certification'] ) ) {
                    $age_rating = $res['release_dates'][0]['certification'];
                    break;
                }
            }
            // Fallback to first available if US not found
            if ( ! $age_rating ) {
                foreach ( $data['release_dates']['results'] as $res ) {
                    if ( ! empty( $res['release_dates'][0]['certification'] ) ) {
                        $age_rating = $res['release_dates'][0]['certification'];
                        break;
                    }
                }
            ...
            foreach ( $data['content_ratings']['results'] as $res ) {
                if ( $res['iso_3166_1'] === 'US' && ! empty( $res['rating'] ) ) {
